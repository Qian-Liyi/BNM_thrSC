...
def run_optimization_core(
    ...
    print(
        f"[GPU {gpu_id}] Processing {len(subject_chunk)} subjects ({sc_mode} SC) at Density {threshold:.2f}..."
    )
    ...
    if batch_problems:
        try:
            ...
            print(f"[GPU {gpu_id}] Density {threshold:.2f} completed.")
        except RuntimeError as e:
            print(
                f"[GPU {gpu_id}] Runtime Error (Atlas: {atlas}, Threshold: {threshold:.2f}): {str(e)}"
            )
